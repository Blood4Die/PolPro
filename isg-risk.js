(function () {
  ...
  const severityOptions = [
    ['1', '1 — Ramak kala / etkisiz'], ['3', '3 — Hafif yaralanma'], ['7', '7 — Ciddi yaralanma'],
    ['15', '15 — Kalıcı hasar'], ['40', '40 — Ölümcül'], ['100', '100 — Birden fazla ölüm']
  ];
  ...
  const defaultTeam = () => ['İşveren vekili', 'İSG uzmanı', 'Çalışan temsilcisi'].join(', ');
  ...
  function renderStep() {
    const titles = ['Tehlike', 'İlk risk', 'Önlem', 'Artık risk ve onay'];
    ...
    if (step === 0) {
      html = `<div class="ohs-form-grid">
        ...
        <label class="full">Tehlike ve olası sonuç<textarea name="hazardDescription" rows="3" required>${fieldValue('hazardDescription')}</textarea></label>
        <label>Etkilenebilecek kişiler<select name="affectedPeople">${textOptions(['Çalışanlar', 'Çalışanlar ve taşeronlar', 'Ziyaretçiler', 'Tüm saha personeli'], draft.affectedPeople)}</select></label>
        ...
        <label class="full">Mevcut önlemler<textarea name="existingControls" rows="3" required>${fieldValue('existingControls')}</textarea></label>
      </div>`;
    } else if (step === 1) {
      html = `<div class="ohs-form-grid">
  ...
  function validateStep(stepIndex, finalSubmission = false) {
    ...
    if (stepIndex === 0 && (!draft.area?.trim() || !draft.hazardDescription?.trim() || !draft.existingControls?.trim() || !draft.evaluationDate || !draft.revision?.trim())) {
      toast('Çalışma alanı, tehlike, mevcut önlemler, tarih ve revizyon bilgileri zorunludur.');
      return false;
    }
    ...
    if (finalSubmission) {
      ...
      if (!draft.area?.trim() || !draft.hazardDescription?.trim() || !draft.existingControls?.trim() || !draft.evaluationDate || !draft.revision?.trim() || !draft.initialScore) {
        toast('Tehlike bilgileri ve ilk risk değerlendirmesi tamamlanmalıdır.');
        return false;
      }
  ...
  async function persistDraft(sendForApproval = false) {
    ...
    for (const file of selectedFiles) {
      try {
        ...
        addActivity(draft.projectId, 'İSG kanıtı yüklendi', file.name, 'file');
      } catch (error) {
    ...
    addActivity(payload.projectId, sendForApproval ? 'İSG riski onaya gönderildi' : `İSG riski ${existing ? 'güncellendi' : 'eklendi'}`, payload.hazardDescription, 'update');
    save();
    $('#ohsRiskDialog').close();
    renderProjectDetail();
    toast(sendForApproval ? 'İSG risk kaydı onaya gönderildi.' : 'İSG risk taslağı kaydedildi.');
  }
  ...
  function openOhsRisk(recordId = null) {
    ...
    step = 0;
    $('#ohsRiskTitle').textContent = existing ? `İSG riskini düzenle · ${existing.revision || 'Rev. 0'}` : 'Yeni İSG risk kaydı';
    renderStep();
  ...
  function renderOhs(p) {
    ...
    root.innerHTML = `<div class="ohs-toolbar"><div><h3>İSG Risk Analizi</h3><p>Tehlike, Fine–Kinney puanı, önlem, artık risk ve doğrulama kayıtları.</p></div><button class="primary permission-create" id="addOhsRisk">+ Yeni risk kaydı</button></div>
      ...
      <div class="ohs-filterbar"><input id="ohsRiskSearch" type="search" placeholder="Faaliyet, tehlike veya sorumlu ara..."><select id="ohsRiskStatus"><option value="">Tüm durumlar</option>${textOptions(['Taslak', 'Onay bekliyor', 'Onaylandı', 'İptal'])}</select></div>
      <article class="panel table-panel"><div class="table-wrap"><table class="enterprise-table ohs-table"><thead><tr><th>No</th><th>Faaliyet / Tehlike</th><th>İlk risk</th><th>Önlem ve sorumlu</th><th>Artık risk</th><th>Durum</th><th></th></tr></thead><tbody id="ohsRiskRows"></tbody></table></div></article>`;

    const renderRows = () => {
      const query = $('#ohsRiskSearch').value.toLocaleLowerCase('tr');
      const selectedStatus = $('#ohsRiskStatus').value;
      const records = allRecords.filter(record => (!selectedStatus || record.approvalStatus === selectedStatus) && (!query || `${record.area} ${record.hazardDescription} ${record.preventiveAction} ${record.owner} ${taskLabel(record.taskId)}`.toLocaleLowerCase('tr').includes(query)));
      $('#ohsRiskRows').innerHTML = records.map((record, index) => {
        ...
        return `<tr class="${record.status === 'İptal' ? 'record-cancelled' : ''}">
          <td><strong>İSG-${String(index + 1).padStart(3, '0')}</strong><small class="ohs-revision">${esc(record.revision || 'Rev. 0')}</small></td>
          <td><strong>${esc(taskLabel(record.taskId))}</strong><small>${esc(record.area || 'Alan belirtilmedi')} · ${esc(record.hazardCategory || 'Diğer')}</small><small>${esc(record.hazardDescription)}</small>${record.workBlocked && record.approvalStatus !== 'Onaylandı' ? '<em class="ohs-blocked">Çalışma başlatılmamalı</em>' : ''}</td>
  ...
  function approveOhsRisk(recordId) {
    if (currentUser?.role !== 'Yönetici') return toast('İSG doğrulamasını yalnızca yönetici tamamlayabilir.');
    ...
    if (!confirm('İSG risk kaydı doğrulanıp kapatılsın mı?')) return;
    ...
    addActivity(record.projectId, 'İSG riski doğrulandı', record.hazardDescription, 'update');
    save();
    renderProjectDetail();
    toast('İSG risk kaydı onaylandı.');
  }

  function cancelOhsRisk(recordId) {
    if (currentUser?.role !== 'Yönetici') return toast('İSG kaydını yalnızca yönetici iptal edebilir.');
    const record = data.ohsRisks.find(item => String(item.id) === String(recordId));
    ...
    addActivity(record.projectId, 'İSG riski iptal edildi', record.hazardDescription, 'update');
    save();
    renderProjectDetail();
    toast('İSG risk kaydı iptal edilerek arşivlendi.');
  }
