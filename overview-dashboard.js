(function () {
  ...
  const storedGanttRange = localStorage.getItem('polpro-dashboard-gantt-range');
  let ganttRange = ['month', '90', '180', 'fit'].includes(storedGanttRange) ? storedGanttRange : 'fit';
  let ganttFilter = 'all';
  ...
  renderDashboardGantt = function () {
    ...
    const continuationClasses = (start, end) => { const startDay = parseDay(start), endDay = parseDay(end); return `${startDay && startDay < bounds.start ? ' continues-left' : ''}${endDay && endDay > bounds.end ? ' continues-right' : ''}`; };
    ...
    visibleProjects.forEach(record => {
      ...
      rows += `<div class="overview-gantt-row project" data-dashboard-expand="${record.id}"><div class="overview-gantt-label"><strong><span class="overview-gantt-toggle">${expanded ? '▾' : '▸'}</span>${escOverview(record.name)}</strong><small>${displayDay(record.start)} – ${displayDay(record.end)}</small></div><div class="overview-gantt-track">${todayPosition}<span class="overview-gantt-bar project${continuationClasses(record.start, record.end)}" style="left:${left}%;width:${Math.max(.4, right - left)}%" title="${displayDay(record.start)} – ${displayDay(record.end)}"><b>%${progress(record.id)}</b></span></div></div>`;
      if (expanded) projectTasks.forEach(task => { const taskLeft = pos(task.start), taskRight = pos(task.end), late = task.status !== 'done' && task.end < today, critical = criticalByProject.get(record.id)?.has(String(task.id)), actualEnd = task.actualEnd || (task.actualStart ? today : ''); rows += `<div class="overview-gantt-row task"><div class="overview-gantt-label"><strong>${escOverview(task.title)}</strong><small>${escOverview(task.assignee || 'Atanmadı')} · ${displayDay(task.start)} – ${displayDay(task.end)}</small></div><div class="overview-gantt-track">${todayPosition}<button type="button" class="overview-gantt-bar ${late ? 'late' : ''} ${critical ? 'critical' : ''}${continuationClasses(task.start, task.end)}" data-dashboard-task="${task.id}" style="left:${taskLeft}%;width:${Math.max(.4, taskRight - taskLeft)}%" title="${escOverview(task.title)} · ${displayDay(task.start)} – ${displayDay(task.end)}"><b>${task.completion || 0}%</b></button>${task.actualStart && actualEnd ? `<i class="overview-actual-bar" style="left:${pos(task.actualStart)}%;width:${Math.max(.25, pos(actualEnd) - pos(task.actualStart))}%"></i>` : ''}</div></div>`; });
    });
    root.innerHTML = `<div class="overview-gantt" style="min-width:${width}px"><div class="overview-gantt-head-row"><div>Proje / görev</div><div class="overview-gantt-months">${months.join('')}${todayPosition}</div></div>${rows || '<div class="overview-gantt-empty">Seçili filtreye uygun plan kaydı bulunmuyor.</div>'}</div>`;
  ...
  function bindDashboardControls() {
    ...
    $$('#dashboardGanttRange [data-dashboard-range]').forEach(button => { button.classList.toggle('active', button.dataset.dashboardRange === ganttRange); button.onclick = () => { ganttRange = button.dataset.dashboardRange; localStorage.setItem('polpro-dashboard-gantt-range', ganttRange); $$('#dashboardGanttRange [data-dashboard-range]').forEach(item => item.classList.toggle('active', item === button)); renderDashboardGantt(); }; });
